...
trap 'logger "process is interrupted...thus Exiting...Task not completed"; exit 1'  2 9
...
sort_data()
{
  ...
  if [ $DEBUG_FLAG -eq 2 ]; then
    ...
    for i in `cat $UNSORTED_FILE`
      do
      logger "`echo $i | awk -F , '{ print "name="$2 "   " "percentage="$1}'`"
    done
  fi

  logger "Sorting file"
  sort -r $UNSORTED_FILE > $UNSORTED_FILE.sorted
...
###############MAIN####################################################
  ...
  while getopts u:s:l:d:? arg
  do
      case $arg in
        u) UNSORTED_FILE=$OPTARG
		   ;;
		s) SORTED_FILE=$OPTARG
		   ;;
		l) DEBUG_LOG_FILE=$OPTARG
           ;;		
        d) DEBUG_FLAG=$OPTARG
		   ;;
        ?) usage
